...
PORTFOLIO_LLM_MODEL = os.getenv("DRAGON_LLM_MODEL", "gpt-5.5")
...
class PortfolioLLM:
    ...
    def get_strategy_system_prompt(self, strategy_type: str) -> str:
        ...
        strategy_rules = {
            "dragon_head": (
                "【龙头战法纪律】\n"
                "1. 推荐列表来自前日涨停龙头，今日寻找回踩低吸机会\n"
                "2. 买入时机：前日龙头今日低开企稳/回踩5日线/分歧转一致时介入\n"
                "3. 高位多板（5板+）大概率高开低走，优先选2-3板刚启动的龙头\n"
                "4. 退潮期（连板股频繁大阴线、亏钱效应扩大）必须空仓，不抄底\n"
                "5. 止盈：盈利8%卖出50%仓位，盈利15%全部清仓\n"
                "6. 持仓周期：1-3天，龙头股最多5天\n"
                "7. 今日推荐的股票是昨日涨停龙头，应评估其今日走势再决定是否买入\n"
            ),
            "sentiment": (
                "【情绪战法纪律】\n"
